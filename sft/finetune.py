...
class ModelArguments:
    ...
    use_flash_attn: Optional[bool] = field(
        default=True,
        metadata={"help": "Whether to enable Flash attention for training."},
    )
    use_peft_lora: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to enable PEFT LoRA for training."},
    )
    ...
    use_8bit_quantization: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to enable loading model in 8bit."},
    )
    use_4bit_quantization: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to enable loading model in 4bit."},
    )
...
class DataArguments:
    ...
    data_path: Optional[str] = field(
        default=None, 
        metadata={"help": "Path to the .jsonl file."}
    )
    ...
    max_length: Optional[int] = field(
        default=None, 
        metadata={"help": "Maximum lenght of the training samples."}
    )
    pad_mode: Optional[str] = field(
        default="pad", 
        metadata={
            "help": "Padding mode for preprocessing the data.",
            "choices": ["pad", "pack"],
        },
    )
...
def main():
    ...
    # Comment out to avoid `ValueError: Must flatten tensors with uniform dtype but got torch.bfloat16 and torch.float32`
    # when using FSDP and PEFT QLoRA
    ...
    if model_args.use_peft_lora:
        ...
        logger.info("=" * 80)
        logger.info(f"trainable params: {n_params:,} || all params: {n_params:,} || trainable%: 100")
        logger.info("=" * 80)
    ...
    # Set `use_orig_params=False` when using FSDP with PEFT
    # Set `fsdp_cpu_ram_efficient_loading=true`, `fsdp_use_orig_params=false` and `fsdp_offload_params=true` (cpu offloading) when using FSDP with PEFT QLoRA
    # refer to https://huggingface.co/docs/peft/v0.15.0/en/accelerate/fsdp
